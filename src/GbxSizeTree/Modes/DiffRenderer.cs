...
public static class DiffRenderer
{
    ...
    public static IAnsiConsole BuildConsole(bool? colorOption) =>
        AnsiConsole.Create(new AnsiConsoleSettings
        {
            ...
            Ansi = colorOption switch { false => AnsiSupport.No, true => AnsiSupport.Yes, null => AnsiSupport.Detect },
    ...
    private static string Compact(string value, bool embedded, bool item) { var s = value; if (embedded && s.StartsWith("Embedded/items/", StringComparison.OrdinalIgnoreCase)) s = s[15..]; if (item && s.Contains('|')) s = s[..s.IndexOf('|')]; return s; }
    ...
    private static void AppendMarkdown(StringBuilder b, string title, IReadOnlyList<Change> changes, bool embedded, bool item, bool coordinates) { if (changes.Count == 0) return; b.Append($"\n### {title}\n\n| Mark | Path |" + (coordinates ? " Coord | Pos |" : "") + (embedded ? " Size |" : "") + "\n|---|---|" + (coordinates ? "---|---|" : "") + (embedded ? "---|" : "") + "\n"); foreach (var c in changes) { var v = c.Right ?? c.Left ?? c.Key ?? "change"; b.Append($"| {(c.Left is null ? "+" : c.Right is null ? "-" : "~")} | {Compact(v, embedded, item)} |"); if (coordinates) b.Append($" {Field(v, "coord") ?? "--"} | {Field(v, "position") ?? Field(v, "pos") ?? "--"} |"); if (embedded) b.Append($" {EmbeddedSizeFormatter(v)} |"); b.AppendLine(); } }
